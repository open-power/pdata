...
static bool name_list_find(struct name_list *nlist, const char *name)
{
	void *r;
...
bool attr_list_parse(const char *attrdb, struct name_list *alist)
{
	FILE *fp;
	char line[128];
	...
	fp = fopen(attrdb, "r");
	if (!fp) {
		fprintf(stderr, "Failed to read attributes from %s\n", attrdb);
		return false;
	}
...
bool attr_list_exists(struct name_list *alist, const char *attr)
{
